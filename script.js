...
//получение блока с редактированием
const editContainer = document.querySelector('.edit');
//получение кнопки сохранить
const saveBtn = document.querySelector('.save-btn');
//получение списка select
const selectList = document.querySelector('.view-column__select-list');
//получение кнопки показать колонку
const selectShowBtn = document.querySelector('.view-column-show');
//получение кнопки скрыть колонку
const selectHideBtn = document.querySelector('.view-column-hide');
//получение коллекции кнопок пагинации
const paginationList = document.querySelector('.pagination__list');

let personId;
const personOnPage = 10;
const selectListValues = ['firstName', 'lastName', 'about', 'eyeColor'];
//работал без локального сервера, поэтому данные загрузил в этот файл
let data = [
    {
...
//Изменение стилей активной кнопки пагинации
const activeBtn = btn => {
	document.querySelectorAll('.pagination__item').forEach(item => item.classList.remove('active'));
	btn.classList.add('active');
};
//Создание новой строки
const creatLinesArray = notes => {
	const array = notes.map(note => {
	  	return (`<tr><td id="firstName" data-id="${note.id}">${note.name.firstName}</td><td id="lastName" data-id="${note.id}">${note.name.lastName}</td><td class="aboutTD" id="about" data-id="${note.id}"><span class="about" data-id="${note.id}">${note.about}</span></td><td id="eyeColor" data-id="${note.id}" class="${note.eyeColor}">${note.eyeColor}</td></tr>`)
	}).join('');

	selectListValues.forEach(item => {
		viewColumn(item, 'show');
	});
	//вывод таблицы на экран
  	tableBody.innerHTML = array;
};
//Показ страниц при переключении через пагинацию
const showPage = event => {
	const target = event.target ? event.target : event;

	activeBtn(target);
	//получение номера активной страницы
  	const pageNum = +target.innerHTML;
	//передача аргументов в функцию -Создание новой строки-
	//(pageNum - 1) * personOnPage - начальная позиция для slice
	//((pageNum - 1) * personOnPage) + personOnPage)- крайняя позиция для slice
  	creatLinesArray(data.slice((pageNum - 1) * personOnPage, ((pageNum - 1) * personOnPage) + personOnPage));
	//Добавление каждой строке слушателя на КЛИК и вызов функции редактирования ячеек
	[...tableBody.rows].forEach(item => {
		item.addEventListener('click', editCells);
	});
};
//Динамическое создание кнопок пагинации
const createPaginationItems = () => {
	//создание пустого массива для кнопок пагинации
	let paginationItemsArray = [];
	//через цикл вычисляем нужное количество кнопок и добавлям их разметку в массив -paginationItemsArray-
	for(let i = 1; i <= Math.ceil(data.length / 10); i++){
		paginationItemsArray.push(`<li class="pagination__item">${i}</li>`);
	};
	//выводим всю эту красоту на экран
	paginationList.innerHTML = paginationItemsArray.join('');
	//каждой кнопке вышаем слушателя на КЛИК и вызов функции показа нужной страницы
	[...document.querySelectorAll('.pagination__item')].forEach(item => item.addEventListener('click', showPage));
};
//Сортировка по столбцам
const sortTable = index => {
	//стандартная функция для метода -sort()-
	const rowSort = (first, second) => {
		const firstRow = first.cells[index].innerHTML;
		const secondRow = second.cells[index].innerHTML;

		if(firstRow < secondRow) return -1;
		else if(firstRow > secondRow) return 1;
		return 0;
	};
	//получаем все ряды текущей страницы
	let rows = [...tableBody.rows];
	//сортируем их
	rows.sort(rowSort);
	//удаляем текущее тело таблицы
	table.removeChild(tableBody);
	//создаём новое, уже отсортированное тело таблицы
	rows.forEach(item => {
...
//каждому заголовку таблицы вешаем слушателя на КЛИК и вызов функции сортировки
table.addEventListener('click', event => {
	const target = event.target;
	//получем индекс текущей колонки
	let index = target.cellIndex;
	//проверяем что мы нажали именно на заголовок
	if(!target.classList.contains("table__thead-item")) return;
	//вызываем функцию для сортировки и передаём индекс столбца, по которому будет сортировка
	sortTable(index);
});
//Функция очистки полей после сохранения редактирования
const clearEdit = () => {
	//получаем все поля и обнуляем их
	document.querySelector(".firstName-input").value = '';
	document.querySelector(".lastName-input").value = '';
	document.querySelector(".about-textarea").value = '';
	document.querySelector(".eyeColor-input").value = '';
};
//Функция редактирования ячеек
const editCells = event => {
  //показываем окно редактирования на экране
	editContainer.style.display = 'block';
  //получаем id человека через созданный заранее атрибут data-id
	personId = event.target.dataset.id;
  //получаем ряд
  let row;
  if(event.target.className === 'about') row = event.target.parentNode.parentNode.childNodes;
  else row = event.target.parentNode.childNodes;
  //получаем значение из таблицы в переменные
	const firstName = [...row][0].innerHTML;
	const lastName = [...row][1].innerHTML;
	const about = [...[...row][2].children][0].innerHTML;
	const eyeColor = [...row][3].innerHTML;
	//выводим значения в поля редактирования
	document.querySelector(".firstName-input").value = firstName;
	document.querySelector(".lastName-input").value = lastName;
	document.querySelector(".about-textarea").value = about;
	document.querySelector(".eyeColor-input").value = eyeColor;
}
//Функция сохранения изменений
const saveChanges = id => {
  //переменная для индекса страницы
	let paginationNum;
  //заменяем значения в БД у человека на новые
	data = data.map(item => {
		if(item.id === id){
			return {
				"id": item.id,
				...
				"phone": item.phone,
				"about": document.querySelector(".about-textarea").value,
				"eyeColor": document.querySelector(".eyeColor-input").value
			}
		} else {
			return item;
		}
	});
  //получаем индекс текущей страницы и заносим в переменную
	[...document.querySelectorAll('.pagination__item')].forEach(item => {
		if(item.classList.contains('active')) paginationNum = item.innerHTML - 1;
	});
  //перерисовываем старницу уже с новыми данными и показываем нужную страницу
	showPage([...document.querySelectorAll('.pagination__item')][paginationNum]);
  //очищаем поля
	clearEdit();
  //прячем окно редактирования
	editContainer.style.display = 'none';
}
//Кривая функция на костылях, которая показывает или скрывает столбцы
const viewColumn = (column, view) => {
	//создание переменных для свойства display и индекс выбранной колонки
	let dispaleValue, columnIndex;
	...
	//присвоение переменной dispaleValue нужное значение
	if (view === 'show') dispaleValue = "table-cell";
	else dispaleValue = "none";
	//получение массива строк
  	const [...tds] = document.querySelectorAll('tr');

  	for(let i = 0; i < tds.length; i++){
		//получение одной ячейки и её скрытие/показ
		if(i === 0) tds[i].childNodes[columnIndex].style.display = dispaleValue;
		else tds[i].childNodes[columnIndex / 2].style.display = dispaleValue;
	}
};
//создаём пагинацию
createPaginationItems();
//показываем первую страницу пользователей при первом запуске
showPage([...document.querySelectorAll('.pagination__item')][0]);
//добавляем слушателя на КЛИК для кнопки сохранения изменений
saveBtn.addEventListener('click', () => saveChanges(personId));

selectShowBtn.addEventListener('click', () => {
	viewColumn(selectList.value, 'show')
});
selectHideBtn.addEventListener('click', () => {
	viewColumn(selectList.value, 'hide');
});